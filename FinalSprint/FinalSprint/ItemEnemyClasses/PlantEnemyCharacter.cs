    ...
    class PlantEnemyCharacter : EnemyCharacter
    {
        ...
        public override void Update(float timeOfFrame)
        {
            if (Parameters.Velocity.Y == 0)
            {
                ...
                if(Math.Abs(Sprint5Main.Game.Scene.Mario.GetMinPosition.X - Parameters.Position.X) <= 100 
            ...
            {
                if (Sprint5Main.Game.Scene.Mario.IsDied())
            ...
            ReadyToTrigger = Math.Abs(Sprint5Main.Game.Scene.Mario.GetMinPosition.X - Parameters.Position.X) > 100 || ReadyToTrigger;
