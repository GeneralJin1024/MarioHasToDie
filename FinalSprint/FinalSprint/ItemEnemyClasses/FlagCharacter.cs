...
namespace FinalSprint.ItemClasses
{
    class FlagCharacter : ItemCharacter
    {
        ...
        public override Sprint5Main.CharacterType Type { get; set; } = Sprint5Main.CharacterType.Flag;

        public FlagCharacter(Texture2D texture, Point rowsAndColumns, Vector2 location): base(texture, rowsAndColumns,location)
        {
            flag = new AnimatedSprite(Sprint5Main.Game.Content.Load<Texture2D>("ItemSprite/flag"), new Point(1, 1), new MoveParameters(false));
