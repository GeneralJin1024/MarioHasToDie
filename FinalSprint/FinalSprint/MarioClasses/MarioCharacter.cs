...
namespace FinalSprint.MarioClasses
{
    public class MarioCharacter : ICharacter
    {
        ...
        public Sprint5Main.CharacterType Type { get; set; } = Sprint5Main.CharacterType.Mario;
        ...
        public void Update(float timeOfFrame)
        {
            ...
            if (Parameters.Position.Y == Stage.Boundary.Y)
            {
                if (!IsDied()) //falling cliff
                    ...
                    Sprint5Main.Game.LevelControl.MarioDied();
            }
        }
        ...
        public void CollideWith(ICharacter character, bool UpOrDown, bool movingDown)
        {
            ...
            {
                case Sprint5Main.CharacterType.Block: CollideWithBlock(UpOrDown, movingDown); break;
                case Sprint5Main.CharacterType.DiedEnemy: CollideWithBlock(UpOrDown, movingDown); break;
                case Sprint5Main.CharacterType.Pipe: CollideWithPipe((PipeCharacter)character, UpOrDown, movingDown); break;
                case Sprint5Main.CharacterType.RedMushroom: CollideWithRedMushRoom(); break;
                case Sprint5Main.CharacterType.Flower: CollideWithFlower(); break;
                case Sprint5Main.CharacterType.Enemy: CollideWithEnemy(UpOrDown && movingDown, character); break;
                case Sprint5Main.CharacterType.Flag: CollideWithFlag(character); break;
                case Sprint5Main.CharacterType.Star: ChangeToStarState(100); break;
                case Sprint5Main.CharacterType.GreenMushroom: Sprint5Main.MarioLife++; break;
                case Sprint5Main.CharacterType.Castle:
                    Parameters.IsHidden = true;
                    Sprint5Main.Game.LevelControl.ChangeToWinMode();
                    break;
                case Sprint5Main.CharacterType.JumpMedicine: Mario.JumpTwice = true; JumpClock = 100; break;
                case Sprint5Main.CharacterType.Bomb:
                    Mario.MarioState.Destroy();
                    if (!IsDied())
                    {
                        ChangeToStarState(15);
