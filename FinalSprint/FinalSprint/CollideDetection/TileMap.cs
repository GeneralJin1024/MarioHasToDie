...
namespace FinalSprint.CollideDetection
{
    public class TileMap
    {
        ...
        private void SearchEntityObjects(ArrayList collideObjects, Sprint5Main.CharacterType type, int row, int column)
        {
            foreach (ICharacter character in Entities[row * MapSize.X + column])
            {
                if (!collideObjects.Contains(character) && !character.Parameters.IsHidden)
                {
                     ...
                     */
                    if (type == Sprint5Main.CharacterType.Mario) //Mario collide with anything that is not hidden
                        collideObjects.Add(character);
                    else if(character.Type == Sprint5Main.CharacterType.Block || character.Type == Sprint5Main.CharacterType.Pipe)
                        collideObjects.Add(character);
                }
            }
        }
        ...
        public void SetEntities(ArrayList characterList)
        {
            ...
            foreach (ICharacter character in characterList)
            {
                ...
                for (int row = minGridPosition.Y; row <= maxGridPosition.Y; row++)
                {
                    ...
                    {
                        if ((character.Type == Sprint5Main.CharacterType.Block || character.Type == Sprint5Main.CharacterType.Coin ||
                            character.Type == Sprint5Main.CharacterType.Pipe || character.Type == Sprint5Main.CharacterType.Flower) &&
                            !Entities[column + MapSize.X * row].Contains(character))
                        {
                            Entities[column + MapSize.X * row].Add(character);
                        }
                        else if (!MovingEntities[column + MapSize.X * row].Contains(character) && character.Type != Sprint5Main.CharacterType.Null)
